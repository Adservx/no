...
export const CustomOrderPDFContactSheet: React.FC<CustomOrderPDFContactSheetProps> = ({ config }) => {
  ...
  const visualizeAsGrid = (pages: number[], cols: number, rows: number): void => {
  ...
  const getCustomPageOrder = (totalPages: number, columns: number, rows: number): number[][] => {
    ...
    for (let sheetIndex = 0; sheetIndex < maxSheetCount; sheetIndex++) {
      ...
      if (sheetIndex < evenSheetCount) {
        const evenSheetPages: number[] = [];
        
        // Create a temporary grid to help visualize the ordering
        const tempGrid: number[][] = Array(rows).fill(0).map(() => Array(columns).fill(0));
        ...
        <>
          ...
          <div className="pdf-preview">
            ...
            <div className="custom-order-info">
              <p>This will create contact sheets with the following page ordering pattern:</p>
              <ul>
      ...
      {showConfirmDialog && (
        <div className="confirmation-dialog">
          <div className="confirmation-content">
            ...
            <p>The custom order contact sheet will be downloaded to your device. Do you want to proceed?</p>
